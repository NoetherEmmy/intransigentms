...
public class Vect implements IVect {
    ...
    public Vect(Collection<Integer> xs) {
    ...
    @Override
    public Vect scalarAdd(int index, int scalar) {
     ...
     ** @throws IndexOutOfBoundsException when {@code index >= dim()} */
    @Override
    public Vect scalarAdd(int index, double scalar) {
        Vect v = new Vect(this);
        v.let(index, v.x(index) + scalar);
        return v;
    }
    ...
    public List<Vect> decomp() {
        List<Vect> decomp = new ArrayList<>(dim());
        for (int i = 0; i < dim(); ++i) {
            Vect v = new Vect(dim());
            v.let(i, elems.get(i));
            decomp.add(v);
     ...
     ** @throws NullPointerException when {@code v == null} */
    @Override
    public Vect directionalProj(IVect v) {
        if (dim() != v.dim()) {
            throw new IllegalArgumentException("Cannot project vectors of mismatched dimensions.");
        }
