...
public class GM implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        } else if (splitted[0].equals("!resetskills")) {
            player.resetAllSkills();
        } else if (splitted[0].equals("!resetdailyprize")) {
            Date ldp = new Date();
            ldp.setTime(0);
            player.setLastDailyPrize(ldp);
        } else if (splitted[0].equals("!sha1")) {
            if (splitted.length > 1) {
                mc.dropMessage(LoginCrypto.hexSha1(splitted[1]));
            }
        } else if (splitted[0].equals("!sendmedamagepacket")) {
        ...
        } else if (splitted[0].equals("!levelpersongrad")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
